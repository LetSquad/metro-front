...
export default function CheckboxField({ name, label, className, required = false }: BaseFieldProps) {
    ...
    return (
        <Form.Field error={isErrorDisplay} required={required} className={className}>
            <div className={styles.container}>
                <Checkbox className={styles.checkbox} label={label} onChange={onClick} checked={value} />
            </div>
            {isErrorDisplay && (
